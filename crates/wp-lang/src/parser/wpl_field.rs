...
fn field_cnt(input: &mut &str) -> ModalResult<usize> {
    preceded(literal('^'), digit1.try_map(str::parse)).parse_next(input)
}
...
pub fn wpl_sep_str(input: &mut &str) -> ModalResult<String> {
    repeat(1.., preceded(literal("\\"), take(1u8))) // TODO 分隔符可能不是一个char，take(1)有问题
...
pub fn wpl_sep(data: &mut &str) -> ModalResult<Option<WplSep>> {
    if peek_str("\\", data).is_ok() {
        let sep = wpl_sep_str.parse_next(data)?;
        if sep.is_empty() {
            fail.context(ctx_desc("sep less")).parse_next(data)?;
        }
        return Ok(Some(WplSep::field_sep(sep)));
    }
    Ok(None)
}
...
fn wpl_field_fmt(input: &mut &str) -> ModalResult<WplFieldFmt> {
    ...
    if peek_str("^", input).is_ok() {
        let cnt = field_cnt.parse_next(input)?;
        return Ok(WplFieldFmt {
            field_cnt: Option::from(cnt),
            ..Default::default()
        });
    }
...
fn wpl_id_field<'a>(input: &mut &'a str) -> ModalResult<(&'a str, WplField)> {
    ...
    /*
    let (meta_type, f_meta) = preceded(multispace0, opt(take_key).map(|x| x.unwrap_or("chars")))
        .try_map::<_, _, MetaErr>(|x| {
            let meta = Meta::from(x)?;
            Ok((meta, x))
        })
        .parse_next(input)?;
        */
...
fn wpl_field_impl(input: &mut &str) -> ModalResult<WplField> {
    ...
    /*
    match conf.meta_type {
        Meta::KV | Meta::ProtoText => conf.use_sep(PrioSep::infer_low(" ")),
        Meta::Json => conf.use_sep(PrioSep::infer_low("0")),
        _ => {}
    }
...
mod tests {
    ...
    #[test]
    fn test_separator() {
        assert_eq!(wpl_sep_str.parse("\\!"), Ok("!".to_string()));
        assert_eq!(wpl_sep_str.parse("\\!\\~"), Ok("!~".to_string()));
        assert_eq!(wpl_sep_str.parse("\\!\\\""), Ok("!\"".to_string()));
        assert_eq!(wpl_sep_str.parse(r#"\!\""#), Ok("!\"".to_string()));
    }
    ...
    fn test_parse_end() {
        ...
        //not field  end
        let conf = wpl_field_fmt.parse("^3").assert();
        assert_eq!(conf.field_cnt, Some(3));
    }
    #[test]
    fn test_wpl_sep() {
