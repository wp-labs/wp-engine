...
use crate::parser::wpl_field::wpl_sep_str;
...
pub fn wpl_group(input: &mut &str) -> WResult<WplGroup> {
    ...
    if peek_str("(", input).is_err() {
        let meta_str = alt(("alt", "opt", "some_of", "seq"))
    ...
    let group_sep = opt(wpl_sep_str).parse_next(input)?;
...
mod tests {
    ...
    fn test_parse_group() {
        ...
        let group = wpl_group
            .parse(
                r#"(ip:sip,_^2,time<[,]>,http/request",http/status,digit,chars",http/agent",_")"#,
            )
            .assert();
    ...
    fn test_parse_group2() {
        let group = wpl_group
            .parse(
                r#"seq(ip:sip,_^2,time<[,]>,http/request",http/status,digit,chars",http/agent",_")"#,
            )
            .assert();
        assert_eq!(group.fields.len(), 9);
        ...
        let group = wpl_group
            .parse(
                r#"opt (ip:sip,_^2,time<[,]>,http/request",http/status,digit,chars",http/agent",_")"#,
            )
            .assert();
        assert_eq!(group.fields.len(), 9);
    }
