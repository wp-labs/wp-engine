...
mod tests {
    ...
    fn test_char2() {
        let mut data = "abc efg hij xxx yyy";
        let conf = WplField::try_parse(r#"chars^2\\\s"#).assert();
        let res = ParserTUnit::new(CharsP::default(), conf)
            .verify_parse_suc(&mut data)
            .assert();
        assert_eq!(res[0], DataField::from_chars("chars", "abc efg"));

        let mut data = "abc efg hij xxx yyy";
        let conf = WplField::try_parse(r#"chars:src^3\\\s"#).assert();
        let res = ParserTUnit::new(CharsP::default(), conf)
            .verify_parse_suc(&mut data)
            .assert();
        assert_eq!(res[0], DataField::from_chars("src", "abc efg hij"));
    }
