# 重构检查清单

> 每个阶段完成后勾选，确保重构进度可追踪

**项目**: CLI 架构简化重构
**开始日期**: 2026-01-10
**负责人**: -

---

## ✅ 阶段 0: 准备工作

- [x] 0.1 创建功能分支 `refactor/simplify-cli-architecture`
- [x] 0.2 记录测试基线（docs/refactor-baseline.md）
- [x] 0.3 记录公共 API 契约（docs/api-contracts.md）
- [ ] 0.4 创建本检查清单（docs/refactor-checklist.md）
- [ ] 0.5 补充集成测试
  - [ ] Source 统计集成测试
  - [ ] Sink 验证集成测试
- [ ] 0.6 提交阶段 0 所有更改

**完成标准**:
- 所有测试通过
- 基线文档完整
- API 契约已记录
- 集成测试已添加

---

## ✅ 阶段 1: 统一参数合并逻辑

**目标**: 消除 3 处重复的参数合并代码

- [x] 1.1 在 `wp-config/src/connectors/` 创建 `params.rs`
- [x] 1.2 实现统一的 `merge_params()` 函数
- [x] 1.3 更新 `wp-cli-core/connectors/sources.rs` 使用新函数
- [x] 1.4 更新 `wp-cli-utils/sources.rs` 使用新函数
- [x] 1.5 检查其他调用点（发现 wp-config 内部实现保留）
- [x] 1.6 运行测试确保行为一致
- [x] 1.7 更新文档
- [x] 1.8 提交阶段 1 更改

**完成标准**:
- ✅ 只有一处统一 `merge_params` 实现
- ✅ 所有 CLI 层调用点使用统一函数
- ✅ 所有测试通过 (911 tests)
- ✅ 无新增编译警告

---

## ⏳ 阶段 2: 移除业务逻辑下沉

**目标**: 将 wp-cli-utils 中的业务逻辑移到 wp-cli-core

- [ ] 2.1 在 `wp-cli-core/src/business/` 创建 `observability` 模块
- [ ] 2.2 将 `wp-cli-utils/sources.rs` 中的业务函数移到 core
  - [ ] `list_file_sources_with_lines()`
  - [ ] `total_input_from_wpsrc()`
- [ ] 2.3 更新 `wp-cli-core/obs/stat.rs` 直接实现逻辑
- [ ] 2.4 移除 `wp-cli-utils/sources.rs` 业务代码
- [ ] 2.5 更新所有调用点
- [ ] 2.6 运行测试
- [ ] 2.7 提交阶段 2 更改

**完成标准**:
- wp-cli-utils 不再包含业务逻辑
- 所有功能正常工作
- 所有测试通过

---

## ⏳ 阶段 3: 缩短调用链

**目标**: 减少不必要的中间层调用

- [ ] 3.1 重写 `stat_src_file()` 直接调用 config 层
- [ ] 3.2 重写 `stat_sink_file()` 直接调用 config 层
- [ ] 3.3 移除不必要的包装函数
- [ ] 3.4 更新所有调用点
- [ ] 3.5 运行集成测试
- [ ] 3.6 提交阶段 3 更改

**完成标准**:
- 调用链深度从 5 层减少到 2-3 层
- 代码可读性提升
- 所有测试通过

---

## ⏳ 阶段 4: 创建新目录结构

**目标**: 在 wp-cli-core 中建立清晰的模块结构

- [ ] 4.1 创建 `wp-cli-core/src/business/` 目录
- [ ] 4.2 创建 `wp-cli-core/src/utils/` 目录
- [ ] 4.3 规划模块布局
  - `business/connectors/`
  - `business/observability/`
  - `utils/pretty/`
  - `utils/validate/`
  - `utils/fs/`
  - `utils/stats/`
- [ ] 4.4 创建必要的 `mod.rs` 文件
- [ ] 4.5 提交目录结构

**完成标准**:
- 新目录结构清晰
- 模块组织合理
- 编译通过

---

## ⏳ 阶段 5: 迁移代码

**目标**: 将 wp-cli-utils 的代码迁移到 wp-cli-core

- [ ] 5.1 迁移 `pretty/` 模块到 `wp-cli-core/src/utils/pretty/`
- [ ] 5.2 迁移 `fsutils.rs` 到 `wp-cli-core/src/utils/fs/`
- [ ] 5.3 迁移 `validate.rs` 到 `wp-cli-core/src/utils/validate/`
- [ ] 5.4 迁移 `stats.rs` 到 `wp-cli-core/src/utils/stats/`
- [ ] 5.5 迁移 `banner.rs` 到 `wp-cli-core/src/utils/banner.rs`
- [ ] 5.6 迁移 `scan.rs` 到对应模块
- [ ] 5.7 更新 `wp-cli-core/src/lib.rs` 导出
- [ ] 5.8 更新所有 `use` 语句
- [ ] 5.9 运行测试
- [ ] 5.10 提交迁移更改

**完成标准**:
- 所有文件已迁移
- 新的模块路径生效
- 所有测试通过
- 无编译警告

---

## ⏳ 阶段 6: 清理和验证

**目标**: 删除旧 crate，完成重构

- [ ] 6.1 删除 `crates/wp-cli-utils/` 目录
- [ ] 6.2 更新 workspace `Cargo.toml`
- [ ] 6.3 更新 `wp-cli-core/Cargo.toml` 依赖
- [ ] 6.4 运行全量测试套件
- [ ] 6.5 运行 clippy 检查
- [ ] 6.6 更新文档
  - [ ] 更新 README（如有必要）
  - [ ] 更新架构文档
- [ ] 6.7 对比 API 契约，确保兼容
- [ ] 6.8 最终提交
- [ ] 6.9 合并到主分支

**完成标准**:
- wp-cli-utils crate 已删除
- 所有测试通过（通过率 100%）
- 无编译警告
- 文档已更新
- API 保持兼容

---

## 最终验证清单

重构完成后，执行以下验证：

- [ ] 运行 `cargo test --workspace` - 所有测试通过
- [ ] 运行 `cargo clippy --workspace` - 无警告
- [ ] 运行 `cargo doc --workspace --no-deps` - 文档构建成功
- [ ] 对比 `docs/refactor-baseline.md` - 代码行数对比
- [ ] 对比 `docs/api-contracts.md` - API 兼容性检查
- [ ] 手动测试关键功能
  - Source 统计
  - Sink 路由验证
  - Connector 列表

---

## 回滚准备

### 如何回滚到特定阶段

```bash
# 查看提交历史
git log --oneline --grep="refactor(phase"

# 回滚到特定阶段
git reset --hard <phase-commit-hash>

# 或创建新分支重新开始
git checkout -b refactor/retry-from-phase-N <phase-commit-hash>
```

### 紧急回滚

```bash
# 回到 develop/1.8
git checkout develop/1.8
git branch -D refactor/simplify-cli-architecture

# 重新开始
git checkout -b refactor/simplify-cli-architecture develop/1.8
```

---

## 风险监控

每个阶段完成后检查：

- [ ] 编译时间变化 < 10%（如适用）
- [ ] 测试通过率 = 100%
- [ ] 代码覆盖率没有下降
- [ ] 没有新的编译警告
- [ ] API 兼容性保持

---

## 里程碑

- [x] **阶段 0 完成**: 准备工作 ✅ (2026-01-10)
- [x] **阶段 1 完成**: 统一参数合并逻辑 ✅ (2026-01-10)
- [ ] **阶段 2 完成**: 移除业务逻辑下沉
- [ ] **阶段 3 完成**: 缩短调用链
- [ ] **阶段 4 完成**: 创建新目录结构
- [ ] **阶段 5 完成**: 迁移代码
- [ ] **阶段 6 完成**: 清理和验证
- [ ] **最终验证**: 所有目标达成 🎯

---

## 注意事项

1. **每阶段提交**: 完成一个阶段后立即提交，便于回滚
2. **测试优先**: 任何更改后都要运行测试
3. **文档同步**: 及时更新相关文档
4. **沟通**: 遇到问题及时讨论，不要独自决定重大变更

---

**最后更新**: 2026-01-10
**状态**: 🟢 进行中 - 阶段 1 已完成
